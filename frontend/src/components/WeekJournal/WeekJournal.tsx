...
import { changeWeekJournal, deleteWeekJournal, plusWeekJournal } from "../../store/weekJournal"

const WeekJournal = () => {

    const [open, setOpen] = useState(true)
    const dispatch = useAppDispatch()
    const { journal, journalCount, journalLastEditTime } = useAppSelector((state) => state.weekJournal)
            ...
            {open &&
                <div className = "mt-4">
                            ...
                            <div key = {i} className = {`border-2 px-2 py-1 ${selectedColor == "blue" && "border-sky-300 bg-sky-50"} ${selectedColor == "whiteBlack" && "border-gray-400/80 bg-gray-50"} ${selectedColor == "pink" && "border-pink-300 bg-pink-50"} rounded-xl shadow-lg mt-6`}>
                                <div className = "flex justify-between items-center">
                                    ...
                                    <div
                                        ...
                                        onClick = {() => dispatch(deleteWeekJournal(i))}
                                    >
                                ...
                                <div className = "mt-2">
                                    <textarea 
                                        ...
                                        rows = {5}
                                        value = {_journal}
                                        onChange = {(e) => dispatch(changeWeekJournal({ text: e.target.value, i: i }))}
                                    />
                                    <textarea 
                                ...
                                <div className = "hidden md:flex justify-start items-end ml-2 my-1 gap-x-2">
                                    <p className = "ml-2 mt-1 text-gray-500">
                                        最終更新日 :
                                    </p>
                    ...
                    <div className = "mt-4 flex flex-col items-center justify-center">
                        <svg onClick = {() => dispatch(plusWeekJournal())} className = "size-10 cursor-pointer text-white p-1 rounded-xl shadow-lg bg-red-400 hover:bg-red-500 hover:scale-110 transition duration-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
