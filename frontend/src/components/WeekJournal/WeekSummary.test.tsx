...
describe("WeekSummaryのテスト", () => {
    ...
    it("初回レンダリング時、１週間分のデータ取得 => サマリーが計算されて、dispatchが呼ばれるか", async () => {
        // 1週間分のダミーデータを用意
        const dummyDailyData = Array.from({ length: 7 }).map((_, i) => ({
            targetDate: `2026-03-0${i + 2}`,
        ...
        render(<WeekSummary renderingTrigger= {0}/>)
        await waitFor(() => {
            // apiが2種類ともちゃんと呼ばれたのかを確認
            expect(axios.get).toHaveBeenCalledTimes(2)
            // setWeekSummaryが何かしらの配列で呼ばれたかを確認
            expect(mockDispatch).toHaveBeenCalledWith(
                expect.objectContaining({
                    type: "weekJournal/setWeekSummary",
