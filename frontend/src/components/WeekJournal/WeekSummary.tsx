...
import { setWeekSummary } from "../../store/weekJournal"

const WeekSummary = ({ renderingTrigger }: { renderingTrigger: number }) => {

    const [open, setOpen] = useState(true)
    const selectedColor = useAppSelector((state) => state.color.color)
    const { summary, endDate } = useAppSelector((state) => state.weekJournal)
    ...
    useEffect(() => {
        ...
        const calcSummaryData = async() => {
            const data = await getDataForSummary(userId, endDate, 7)
            const yestardaySleepData = await getDataForSumYestardaySleep(userId, endDate, 7)
            const summaryData = calcSummaryState(data, yestardaySleepData, endDate, 7)
            if (summaryData.length > 0) {
                dispatch(setWeekSummary(summaryData))
                setIsFetched(true)
            } else {
                setIsFetched(false)
            }
        ...
        calcSummaryData()
    }, [userId, endDate, dispatch ,renderingTrigger])
    ...
    return (
        <div className = {`border-2 ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-300"} rounded-2xl shadow-lg py-4 px-2 mt-6`}>
            ...
            <div className = {`flex justify-start items-center gap-x-6 ml-2 border-b-2 border-dashed ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-400"} pb-3`}>
                ...
                <h2 className = "text-2xl text-gray-700">今週の進捗</h2>
            </div>
                    ...
                    <div className = "md:flex justify-center items-center my-2">
                        <div className = "md:pr-6">
                            ...
                            <div className = "mt-5 ml-2 flex justify-start items-center gap-x-2">
                                ...
                                <p className = "text-lg">今週やるべきことをやった時間</p>
                            </div>
