...
import PastRoutine from "./CreateRoutine/PastRoutine"
...
const CreateRoutine = () => {
  ...
  const [routineList, setRoutineList] = useState<RoutineInitState[]>([])
  // ルーティーンのデータを取得
  const getRoutineList = useCallback(async() => {
    try {
      ...
      console.log("getRoutineListでエラー", e)
    }
  }, [userId])
  ...
  return (
    <div className = {`mt-4 border-2 mx-3 py-6 px-3 rounded-2xl shadow-lg bg-white border-dashed ${selectedColor !== "pink" && "border-gray-500"} ${selectedColor == "pink" && "border-pink-500"}`}>
      ...
      >
        ...
        <p className = {`text-xl ${selectedColor !== "pink" && "text-gray-700"} ${selectedColor == "pink" && "text-pink-700"}`}>新しいルーティーンを作成</p>
      </div>
      ...
      <div className = {`flex justify-center items-center gap-x-2 mt-6 border-b-2 pb-2 border-dashed ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-400"} mx-1 text-gray-600`}>
        ...
        <h2 className = "text-xl">過去に作成したルーティーン</h2>
      </div>

      {/* mapメソッドで過去に作成したジャーナルを繰り返す */}
      <div className = "block md:grid justify-center items-center grid-cols-2 gap-x-8 gap-y-6">
        {routineList.map((_routine) => {
          // console.log(_routine)
          return (
            <PastRoutine key = {_routine.routineId} _routine = {_routine} getRoutineList = {getRoutineList} />
          )
        })}
      </div>
