...
import { setCurrentWeek } from "../../store/weekJournal"
import dateToYearMonthDate from "../../func/DateToYearMonthDate"

interface Props {
    journal: WeekJournalInitState,
    handleDeleteSuccess: () => void
}

const DispWeek = ({ journal, handleDeleteSuccess }: Props) => {
    ...
    const editJournal = () => {
        // stateを設定
        dispatch(setCurrentWeek(journal))
        localStorage.setItem("weekJournalStartDate", journal.startDate)
        localStorage.setItem("weekJournalEndDate", journal.endDate)
    ...
    return (
        <div>
            ...
            <div className = {`mt-4 border-2 ${selectedColor == "blue" && "border-sky-400"} ${selectedColor == "whiteBlack" && "border-gray-500"} ${selectedColor == "pink" && "border-pink-400"} rounded-xl shadow-xl`}>
                <div className = {`flex justify-center items-center text-center gap-x-6 ${selectedColor == "blue" && "bg-sky-400"} ${selectedColor == "whiteBlack" && "bg-gray-500"} ${selectedColor == "pink" && "bg-pink-400"} rounded-t-lg rounded-b-md py-3`}>
                    <div className = "text-white text-lg">
                        <p className = "mb-1">{dateToYearMonthDate(journal.startDate)[0]}年</p>
                        <p>{dateToYearMonthDate(journal.startDate)[1]}月 {dateToYearMonthDate(journal.startDate)[2]}日</p>
                    </div>
                    <p className = "text-white text-2xl my-1">〜</p>
                    <div className = "text-white text-lg">
                        <p className = "mb-1">{dateToYearMonthDate(journal.endDate)[0]}年</p>
                        <p>{dateToYearMonthDate(journal.endDate)[1]}月 {dateToYearMonthDate(journal.endDate)[2]}日</p>
                    </div>
                </div>
            ...
            >
                ...
                >
                    <p className = "mb-1">{dateToYearMonthDate(journal.startDate)[0]}年 {dateToYearMonthDate(journal.startDate)[1]}月 {dateToYearMonthDate(journal.startDate)[2]}日 から</p>
                    <p className = "mb-1">{dateToYearMonthDate(journal.endDate)[0]}年 {dateToYearMonthDate(journal.endDate)[1]}月 {dateToYearMonthDate(journal.endDate)[2]}日 までの</p>
