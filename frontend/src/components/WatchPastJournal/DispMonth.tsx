...
import { setCurrentMonth } from "../../store/monthJournal"
import dateToYearMonthDate from "../../func/DateToYearMonthDate"

interface Props {
    journal: MonthJournalInitState,
    handleDeleteSuccess: () => void
}

const DispMonth = ({ journal, handleDeleteSuccess }: Props) => {
        ...
        try {
            await axios.delete("/api/monthJournal/deleteMonthJournal", {
                params: {
                    ...
                    targetDate: journal.targetDate + "-01"
                }
            })
            setBtn(false)
            handleDeleteSuccess()
    ...
    const editJournal = () => {
        // stateを設定
        dispatch(setCurrentMonth(journal))
        localStorage.setItem("monthJournalDate", journal.targetDate)
        ...
        <div>
            ...
            <div className = {`mt-4 border-2 ${selectedColor == "blue" && "border-sky-400"} ${selectedColor == "whiteBlack" && "border-gray-500"} ${selectedColor == "pink" && "border-pink-400"} rounded-xl shadow-xl`}>
                <div className = {`text-center ${selectedColor == "blue" && "bg-sky-400"} ${selectedColor == "whiteBlack" && "bg-gray-500"} ${selectedColor == "pink" && "bg-pink-400"} rounded-t-lg rounded-b-md py-3`}>
                    <p className = "text-white text-lg">{dateToYearMonthDate(journal.targetDate + "-01")[0]}年 {dateToYearMonthDate(journal.targetDate + "-01")[1]}月</p>
                </div>
            ...
            >
                ...
                >
                    <p className = "mb-1">{dateToYearMonthDate(journal.targetDate + "-01")[0]}年 {dateToYearMonthDate(journal.targetDate + "-01")[1]}月</p>
