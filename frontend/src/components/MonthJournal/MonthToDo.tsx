...
import { changeMonthToDo, changeMonthToDoImportant, deleteMonthToDo, plusMonthToDo, toggleMonthToDoCheck } from "../../store/monthJournal"

const MonthToDo = () => {

    const [open, setOpen] = useState(true)
    const { toDo, toDoCheck, toDoImportant } = useAppSelector((state) => state.monthJournal)
    ...
    return (
        <div className = {`border-2 ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-300"} rounded-2xl shadow-lg py-4 px-2`}>
            ...
            <div className = {`flex justify-start items-center gap-x-6 ml-2 border-b-2 border-dashed ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-400"} pb-3`}>
                ...
                <h2 className = "text-2xl text-gray-700">今月達成したいこと</h2>
            </div>
            ...
            {open &&
                <div className = "mt-4">
                            ...
                            <div key = {i} className = "md:pl-4 flex justify-start items-center gap-x-3 mx-2 mt-6">
                                <div>
                                    <input 
                                        ...
                                        checked = {toDoCheck[i]}
                                        onChange = {() => dispatch(toggleMonthToDoCheck(i))}
                                    />
                                    <div 
                                        data-testid = {`delete-button-${i}`}
                                        className = {`mt-4 cursor-pointer ${selectedColor !== "pink" && "bg-gray-200 hover:bg-gray-300 hover:text-gray-700"} ${selectedColor == "pink" && "bg-pink-200 text-pink-600 hover:bg-pink-300 hover:text-pink-700"} p-0.5 rounded-lg shadow-sm hover:scale-110 transition duration-200`}
                                        onClick = {() => dispatch(deleteMonthToDo(i))}
                                    >
                                ...
                                <div className = {`border-l-2 pl-3 ${selectedColor == "blue" && "border-sky-400"} ${selectedColor == "whiteBlack" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-400"} border-dashed`}>
                                    <textarea 
                                        data-testid = {`input-${i}`}
                                        ...
                                        rows = {2}
                                        value = {_toDo}
                                        onChange = {(e) => dispatch(changeMonthToDo({ text: e.target.value, i: i }))}
                                    />
                                    <div className = "flex justify-center items-center gap-x-2 mt-2">
                                        ...
                                        <div 
                                            ...
                                            className = {`text-2xl cursor-pointer hover:text-red-800 hover:scale-110 transition duration-200 ${toDoImportant[i] === 2 ? "text-red-800" : "text-red-200/80"}`}
                                            onClick = {() => dispatch(changeMonthToDoImportant({ value: 2, i: i }))}
                                        >
                                            ◎
                                        </div>
                                        <div 
                                            data-testid = {`important1-${i}`}
                                            className = {`text-2xl cursor-pointer hover:text-rose-800 hover:scale-110 transition duration-200 ${toDoImportant[i] === 1 ? "text-rose-800" : "text-rose-200/80"}`}
                                            onClick = {() => dispatch(changeMonthToDoImportant({ value: 1, i: i }))}
                                        >
                                            ⚪︎
                                        </div>
                                        <div 
                                            data-testid = {`important0-${i}`}
                                            className = {`text-2xl cursor-pointer hover:text-sky-800 hover:scale-110 transition duration-200 ${toDoImportant[i] === 0 ? "text-sky-800" : "text-sky-200/80"}`}
                                            onClick = {() => dispatch(changeMonthToDoImportant({ value: 0, i: i }))}
                                        >
                                            △
                                        </div>
                                    </div>
                    ...
                    <div className = "flex justify-center mt-6 mb-2">
                        <svg onClick = {() => dispatch(plusMonthToDo())} className = {`size-10 cursor-pointer text-rose-500 ${selectedColor !== "pink" && "bg-gray-100 hover:bg-gray-200"} ${selectedColor == "pink" && "bg-pink-100 hover:bg-pink-200"} rounded-md shadow-lg hover:text-rose-600 hover:scale-110 transition duration-200`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
