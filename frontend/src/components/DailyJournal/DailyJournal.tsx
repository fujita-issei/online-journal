...
import { changeJournal, deleteJournal, plusJournal } from "../../store/dailyJournal"

const DailyJournal = () => {

    const [open, setOpen] = useState(true)
    const { journal, journalCount, journalLastEditTime } = useAppSelector((state) => state.dailyJournal)
    ...
    return (
        <div className = {`border-2 ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-300"} rounded-2xl shadow-lg py-4 px-2 mt-6`}>
            ...
            <div className = {`flex justify-start items-center gap-x-6 ml-2 border-b-2 border-dashed ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-400"} pb-3`}>
                ...
                <h2 className = "text-2xl text-gray-700">ジャーナルを執筆</h2>
            </div>
            ...
            {open &&
                <div className = "mt-4">
                    {journal.map((_journal, i) => {
                        return (
                            <div className = {`border-2 px-2 py-1 ${selectedColor == "blue" && "border-sky-300 bg-sky-50"} ${selectedColor == "whiteBlack" && "border-gray-400/80 bg-gray-50"} ${selectedColor == "pink" && "border-pink-300 bg-pink-50"} rounded-xl shadow-lg mt-6`} key = {i}>
                                <div className = "flex justify-between items-center">
                                    ...
                                    <div 
                                        ...
                                        className = {`${selectedColor !== "pink" && "bg-gray-200 hover:bg-gray-300 hover:text-gray-700"} ${selectedColor == "pink" && "bg-pink-200 hover:bg-pink-300 hover:text-pink-700"} cursor-pointer mr-2 mt-1 p-0.5 rounded-lg shadow-sm hover:scale-110 transition duration-200`}
                                        onClick = {() => dispatch(deleteJournal(i))}
                                    >
                                ...
                                <div className = "mt-2">
                                    <textarea 
                                        ...
                                        rows = {8}
                                        value = {_journal}
                                        onChange = {(e) => dispatch(changeJournal({ text: e.target.value, i: i }))}
                                    />
                                    <textarea 
                    ...
                    <div className = "mt-4 flex flex-col items-center justify-center">
                        <svg onClick = {() => dispatch(plusJournal())} className = "size-10 cursor-pointer text-white p-1 rounded-xl shadow-lg bg-red-400 hover:bg-red-500 hover:scale-110 transition duration-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
