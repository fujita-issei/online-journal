...
const MoneyConsumptionGraph = ({ consumptionData }: Props) => {
    ...
    const chartData = [...(Array.isArray(consumptionData) ? consumptionData : [])].reverse().map((data) => {
        const dateStr = new Date(data.targetDate);
        const displayDate = `${dateStr.getFullYear()}/${dateStr.getMonth() + 1}/${dateStr.getDate()}`; // 例: "2/28"

        return {
            date: displayDate,
            consumption: data.consumption // Y軸用のデータ（投資額）
        }
    });
                ...
                >
                    ...
                    <Tooltip 
                        ...
                        formatter={(value: number | undefined) => {
                            ...
                            if (value === undefined) return ["", "消費額"];
                            return [`${value.toLocaleString()} 円`, "消費額"];
                        }}
...
export default MoneyConsumptionGraph;