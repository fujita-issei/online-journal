...
const ToDoAchieveGoalTimePieChart = ({ chartData }: Props) => {
    ...
    // 3. グラフ用のデータを作成
    const pieData = [
        { name: '完了', value: chartData.achieveGoalTime },
        { name: '未完了', value: chartData.all - chartData.achieveGoalTime },
    ];
    ...
    return (
        <div className="w-full h-48 mt-4 relative">
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
                ...
                `}>
                    {Math.round((chartData.achieveGoalTime / chartData.all) * 100)}<span className="text-sm">%</span>
                </p>
            </div>
        </div>
    )
...
export default ToDoAchieveGoalTimePieChart