...
import { changeYearToDo, changeYearToDoImportant, deleteYearToDo, plusYearToDo, toggleYearToDoCheck } from "../../store/yearJournal"

const YearTodo = () => {

    const [open, setOpen] = useState(true)
    const dispatch = useAppDispatch()
    const { toDo, toDoCheck, toDoImportant } = useAppSelector((state) => state.yearJournal)
    ...
    return (
        <div className = {`border-2 ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-300"} rounded-2xl shadow-lg py-4 px-2`}>
            ...
            <div className = {`flex justify-start items-center gap-x-6 ml-2 border-b-2 border-dashed ${selectedColor !== "pink" && "border-gray-400"} ${selectedColor == "pink" && "border-pink-400"} pb-3`}>
                ...
                <h2 className = "text-2xl text-gray-700">今年達成したいこと</h2>
            </div>
                            ...
                            <div key = {i} className = "md:pl-4 flex justify-start items-center gap-x-3 mx-2 mt-6">
                                <div>
                                    <input 
                                        ...
                                        checked = {toDoCheck[i]}
                                        onChange = {() => dispatch(toggleYearToDoCheck(i))}
                                    />
                                    <div 
                                        ...
                                        onClick = {() => dispatch(deleteYearToDo(i))}
                                    >
                                ...
                                <div className = {`border-l-2 pl-3 ${selectedColor == "blue" && "border-sky-400"} ${selectedColor == "whiteBlack" && "border-gray-500"} ${selectedColor == "pink" && "border-pink-400"} border-dashed`}>
                                    <textarea 
                                        ...
                                        rows = {2}
                                        value = {_toDo}
                                        onChange = {(e) => dispatch(changeYearToDo({ text: e.target.value, i: i }))}
                                    />
                                    <div className = "flex justify-center items-center gap-x-2 mt-2">
                                        ...
                                        <div 
                                            ...
                                            onClick = {() => dispatch(changeYearToDoImportant({ value: 2, i: i }))}
                                        >
                                            ◎
                                        </div>
                                        <div 
                                            ...
                                            onClick = {() => dispatch(changeYearToDoImportant({ value: 1, i: i }))}
                                        >
                                            ⚪︎
                                        </div>
                                        <div 
                                            ...
                                            onClick = {() => dispatch(changeYearToDoImportant({ value: 0, i: i }))}
                                        >
                                            △
                                        </div>
                                    </div>
